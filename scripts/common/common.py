...
def border_calc_path(poly, width, bStartTop=False, edge_offset=None):
	ret = []
	edge_buffer = (width/2.0)
	if(edge_offset is not None): edge_buffer = edge_buffer + edge_offset

	# Border path is polygon reduced by width
	poly_inner = poly.buffer(-edge_buffer)
	if poly_inner.is_empty: return ret
	...
	coll.rotate(-min_i)		# Rotate points of polygon to begin start_edge
	coll.append(coll[0])	# append last point again
	return list(coll)
def trapezoid_calc_path(poly, robot_width, bStartTop=False, edge_offset=None):
	ret = []
	up = False
	pos_next = None
	...
	if poly_width < robot_width and poly_height < robot_width: return ret # Don't bother

	# Reduce size of polygon by width
	edge_buffer = (robot_width/2.0)
	if(edge_offset is not None): edge_buffer = edge_buffer + edge_offset
	...
	if robot_width >= poly_width or robot_width >= poly_height:
		...
		yscale = 1-(robot_width/poly_height)-0.1 if robot_width < poly_height else 0.0
		poly_inner = shapely.affinity.scale(poly, xscale, yscale)

	plows = ceil(poly_width/robot_width)
	plow_width = poly_width/plows
	if poly_inner.is_empty: return ret # No polygon left

	for x in np.arange(poly_inner.bounds[0], poly_inner.bounds[2]+plow_width, plow_width):
		if x > poly_inner.bounds[2]: x = poly_inner.bounds[2] # Correct rounding errors

		intersections_y = None # Get both intersections of current x with y
		if poly_inner.bounds[0] == poly_inner.bounds[2]: intersections_y = [poly_inner.bounds[1], poly_inner.bounds[3]] # more a line then polygon
		else:
			try:
		...
		if len(intersections_y) < 2:
			y1 = intersections_y[0] # Only one intersection
			ret.append([x, y1])
		else:
			...
			for i in range(2): # One to follow the current plow, one to drive to the next plow
				...
				pos_cur = pos_next
				if not pos_cur in ret: ret.append(pos_cur)
...
def buildPoseMsg(posList, quatList, flatnav=False):
	if(posList is None): return PoseStamped()
	elif(quatList is None): return PoseStamped()
	...
	return msg
def printActionServerState(state):
...
def getCostmapCellCost(costmap, loc_xy):
	cellx = round((loc_xy[0]-costmap.info.origin.position.x)/costmap.info.resolution)
	celly = round((loc_xy[1]-costmap.info.origin.position.y)/costmap.info.resolution)
	...
	# rospy.logdebug("Local costmap cost = %s at X,Y  = (%.3f, %.3f)" % (str(cost), loc_xy[0], loc_xy[1]) )
	return cost
...
if __name__ == "__main__":
	...
	def test(points, width=0.1, bStartTop=False, bot_width=0.1, edge_offset=None):
		fig, ax = plt.subplots()
		poly = Polygon(points)
		path = trapezoid_calc_path(poly, bot_width, bStartTop, edge_offset=edge_offset)

		pos_last = None
		for pos_cur in path:
			if pos_last is not None: paint_arrow(ax, pos_last, pos_cur, width)
			pos_last = pos_cur
	...
	# test([[0.0, 3.0], [10.0, 1.0], [10.0, 15.0], [0.0, 12.0]])
	# test([[0.0, 3.0], [10.0, 1.0], [10.0, 15.0], [0.0, 12.0]], edge_offset=0.1)
	...
	test([[0.0, 3.0], [10.0, 1.0], [10.0, 15.0], [0.0, 12.0]], bot_width=0.5 )
	test([[0.0, 3.0], [10.0, 1.0], [10.0, 15.0], [0.0, 12.0]], bot_width=0.5, edge_offset=1.0 )
